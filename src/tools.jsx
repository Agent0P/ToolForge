...
`Project Quote
─────────────────
${tasks.filter(t=>t.name).map(t => `${t.name}: ${t.hours}h × ${sym}${rate}/h = ${sym}${Number((Number(t.hours)*Number(rate)).toFixed(0)).toLocaleString()}`).join("
")}
─────────────────
Subtotal:  ${sym}${Number(basePrice.toFixed(0)).toLocaleString()}
Buffer (${buffer}%): ${sym}${Number((withBuffer - basePrice).toFixed(0)).toLocaleString()}${rush ? `
